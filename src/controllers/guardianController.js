...
const getPrimaryGuardianController = async (req, res) => {
  ...
  try {
    ...
    if (!isAdmin) {
      const isTeamMember = await isUserTeamMemberForSameClient(loggedInUserId, clientId);
      if (!isTeamMember) {
        return res.status(403).json({
          message: "Access denied. You are not authorized to view this client's primary guardian information.",
        });
      }
    }
    ...
    getPrimaryGuardianByClientId(clientId, (err, results) => {
      if (err) {
        return res.status(500).json({ message: "Error fetching primary guardian information", error: err });
      }
      if (!results || results.length === 0) {
        return res.status(404).json({ message: "No primary guardian found for the client" });
      }
